...
$admin_dots = $notificationService->getDotCounts('admin');
$staff_dots = $notificationService->getDotCounts('staff');
$supplier_dots = $notificationService->getDotCounts('supplier');
$customer_dots = $notificationService->getDotCounts('customer');
    ...
    <script>
        class HomePageController {
            constructor() {
                ...
                this.guestOrderModal = document.getElementById('guestOrderModal');
                this.floatingNotice = document.getElementById('floatingNotice');

                this.productSearchForm = document.getElementById('productSearchForm');
                this.productSearchInput = document.getElementById('productSearchInput');
                this.productSearchStatus = document.getElementById('productSearchStatus');
                this.productSearchResults = document.getElementById('productSearchResults');
                ...
                this.setupFloatingNotice();
            }

            bindMenuEvents() {
                if (this.menuBtn && this.dropdownMenu && this.menuOverlay) {
            ...
            bindModalEvents() {
                ...
                if (this.guestOrderModal) {
                    this.guestOrderModal.addEventListener('click', (event) => {
                        if (event.target === this.guestOrderModal) {
                            this.closeGuestOrderModal();
                        }
                    });
                }
            }
            ...
            bindSearchEvents() {
                if (!this.productSearchForm || !this.productSearchInput || !this.productSearchStatus || !this.productSearchResults) {
                    console.warn('Search UI elements missing; search disabled.');
                    return;
                }

                this.productSearchForm.addEventListener('submit', async (e) => {
                    e.preventDefault();
                    const query = this.productSearchInput.value.trim();
                    if (!query) {
                        this.setSearchStatus('Enter a product name to search.', 'empty');
                        this.notify('Please type a product name.', 'info');
                        this.productSearchResults.innerHTML = '';
                        return;
                    }

                    this.setSearchStatus('Searching...', 'muted');
                    this.productSearchResults.innerHTML = '';
                    try {
                        ...
                        this.setSearchStatus('Search failed. Please try again.', 'empty');
            ...
            setupFloatingNotice() {
                ...
                const dragHandle = this.floatingNotice.querySelector('.drag-handle');
                let isDragging = false;
                let offsetX = 0;
                let offsetY = 0;

                const placeNoticeBottomRight = () => {
                    const noticeRect = this.floatingNotice.getBoundingClientRect();
                    this.floatingNotice.style.left = `${window.innerWidth - noticeRect.width - 30}px`;
                    this.floatingNotice.style.top = `${window.innerHeight - noticeRect.height - 30}px`;
                };
                ...
                if (dragHandle) {
                    dragHandle.addEventListener('pointerdown', (e) => {
                        isDragging = true;
                        offsetX = e.clientX - this.floatingNotice.getBoundingClientRect().left;
                        offsetY = e.clientY - this.floatingNotice.getBoundingClientRect().top;
                        this.floatingNotice.setPointerCapture(e.pointerId);
                    });
                    ...
                    dragHandle.addEventListener('pointermove', (e) => {
                        ...
                        const maxLeft = window.innerWidth - this.floatingNotice.offsetWidth;
                        const maxTop = window.innerHeight - this.floatingNotice.offsetHeight;
                        this.floatingNotice.style.left = `${Math.min(Math.max(0, newLeft), maxLeft)}px`;
                        this.floatingNotice.style.top = `${Math.min(Math.max(0, newTop), maxTop)}px`;
                    });

                    dragHandle.addEventListener('pointerup', (e) => {
                        isDragging = false;
                        this.floatingNotice.releasePointerCapture(e.pointerId);
                    });
                ...
                window.addEventListener('resize', () => {
                    const rect = this.floatingNotice.getBoundingClientRect();
                    const maxLeft = window.innerWidth - rect.width;
                    const maxTop = window.innerHeight - rect.height;
                    this.floatingNotice.style.left = `${Math.min(parseInt(rect.left, 10), maxLeft)}px`;
                    this.floatingNotice.style.top = `${Math.min(parseInt(rect.top, 10), maxTop)}px`;
                });
            }
            ...
            dismissNotice() {
                if (this.floatingNotice) {
                    this.floatingNotice.style.display = 'none';
                }
            }
            ...
            setSearchStatus(text, cls = 'muted') {
                if (!this.productSearchStatus) {
                    return;
                }
                this.productSearchStatus.textContent = text;
                this.productSearchStatus.className = `search-status ${cls}`.trim();
            }
            ...
            renderResults(items) {
                ...
                if (!Array.isArray(items) || items.length === 0) {
                    this.setSearchStatus('No products found.', 'empty');
                    this.notify('No products found.', 'error');
                    this.productSearchResults.innerHTML = '';
                    return;
                }

                this.setSearchStatus(`${items.length} result${items.length > 1 ? 's' : ''} found`, 'success');
                this.notify(`${items.length} product${items.length > 1 ? 's' : ''} found.`, 'success');
                this.productSearchResults.innerHTML = items.map((item) => {
            ...
            openGuestOrderModal() {
                if (this.guestOrderModal) {
                    this.guestOrderModal.classList.add('active');
                }
                this.resetGuestOrder();
            }

            closeGuestOrderModal() {
                if (this.guestOrderModal) {
                    this.guestOrderModal.classList.remove('active');
                }
            }
            ...
            resetGuestOrder() {
                ...
                const step1 = document.getElementById('guestStep1');
                const step2 = document.getElementById('guestStep2');
                const step3 = document.getElementById('guestStep3');
                if (step1) {
                ...
                const otp = document.getElementById('guestOTP');
                if (name) {
                    name.value = '';
                }
                if (phone) {
            ...
            showGuestStep(stepNum) {
                document.querySelectorAll('.guest-step').forEach((s) => s.classList.remove('active'));
                const step = document.getElementById('guestStep' + stepNum);
                if (step) {
            ...
            async sendGuestOTP() {
                ...
                if (!/^\d{11}$/.test(phone)) {
                    this.notify('Phone must be exactly 11 digits', 'error');
                    return;
                }
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        this.showGuestStep(2);
                        this.notify('OTP sent successfully!', 'success');
                    } else {
                        this.notify(data.message || 'Failed to send OTP', 'error');
                    }
                } catch (err) {
                    this.notify('Error sending OTP', 'error');
                }
            }
            ...
            async verifyGuestOTP() {
                ...
                if (!/^\d{4}$/.test(otp)) {
                    this.notify('OTP must be 4 digits', 'error');
                    return;
                }
                ...
                try {
                    const res = await fetch('guest_order_api.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: `action=verifyOTP&session_id=${this.guestSessionId}&otp=${otp}`
                    });
                    const data = await res.json();

                    if (data.success) {
                        this.notify('OTP verified!', 'success');
                        await this.loadGuestProducts();
                        this.showGuestStep(3);
                    } else {
                        this.notify(data.message || 'Invalid OTP', 'error');
                    }
                } catch (err) {
                    this.notify('Error verifying OTP', 'error');
                }
            }

            async loadGuestProducts() {
                try {
                    const res = await fetch('guest_order_api.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            ...
            renderGuestProducts() {
                ...
                grid.innerHTML = this.guestProducts.map((p) => `
            ...
            updateGuestCart(productId) {
                ...
                input.style.borderColor = qty > 0 && qty < 10 ? '#ff4444' : 'rgba(255,255,255,0.2)';

                if (qty >= 10) {
                    this.guestCart[productId] = { qty, price };
                    card.classList.add('selected');
                } else if (qty === 0) {
                    delete this.guestCart[productId];
            ...
            calculateGuestTotal() {
                ...
                Object.keys(this.guestCart).forEach((id) => {
                    totalStocks += this.guestCart[id].qty;
                    subtotal += this.guestCart[id].qty * this.guestCart[id].price;
                });
                ...
                const totalStocksEl = document.getElementById('guestTotalStocks');
                ...
                if (totalStocksEl) {
                    totalStocksEl.textContent = String(totalStocks);
                }
                if (subtotalEl) {
                ...
                if (summaryDiv) {
                    summaryDiv.style.display = Object.keys(this.guestCart).length > 0 ? 'block' : 'none';
                }

                let valid = totalStocks >= 100;
                Object.keys(this.guestCart).forEach((id) => {
                    if (this.guestCart[id].qty < 10) {
                        valid = false;
                    }
                });
                ...
                if (checkoutBtn) {
                    ...
                    if (totalStocks < 100 && Object.keys(this.guestCart).length > 0) {
        ...
        // Keep inline HTML handlers working while routing through class methods.
        window.selectRole = (role) => homePage.selectRole(role);
        window.dismissNotice = () => homePage.dismissNotice();
        window.openAbout = () => homePage.openAbout();
        window.closeAbout = () => homePage.closeAbout();
        window.openGuestOrderModal = () => homePage.openGuestOrderModal();
        window.closeGuestOrderModal = () => homePage.closeGuestOrderModal();
        window.sendGuestOTP = () => homePage.sendGuestOTP();
        window.verifyGuestOTP = () => homePage.verifyGuestOTP();
        window.updateGuestCart = (productId) => homePage.updateGuestCart(productId);
        window.guestCheckout = () => homePage.guestCheckout();
    </script>
</body>
</html>
