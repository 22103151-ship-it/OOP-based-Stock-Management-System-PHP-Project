...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $user_id = $userModel->create($name, $email, $password, $role);
    if ($user_id > 0) {

        // If creating a customer, also create customer record
        if ($role === 'customer') {
            $customer_id = $customerModel->create($user_id, $name, $email, '', '', 'pro');
            if ($customer_id > 0) {
                echo "<div class='alert-success'>✅ Customer user and profile created successfully!</div>";
            } else {
                echo "<div class='alert-warning'>⚠️ User created but customer profile creation failed.</div>";
            }
        } else {
            echo "<div class='alert-success'>✅ User created successfully!</div>";
        }
    } else {
        echo "<div class='alert-error'>❌ Error creating user.</div>";
    }
}
?>
