...
include '../config.php';
use App\Core\Auth;
use App\Services\SupplierDashboardService;
Auth::requireRole('supplier');
include '../includes/supplier_helpers.php';

$supplier_id = getResolvedSupplierId($conn);
$supplierDashboardService = new SupplierDashboardService($conn);

$counts = $supplierDashboardService->getOrderCounts($supplier_id > 0 ? $supplier_id : null);
$total_orders = $counts['total_orders'] ?? 0;
$pending_orders = $counts['pending_orders'] ?? 0;
$delivered_orders = $counts['delivered_orders'] ?? 0;
$returned_orders = $counts['returned_orders'] ?? 0;

// Get notification dots for supplier
$supplier_dots = $supplierDashboardService->getNotifications();
