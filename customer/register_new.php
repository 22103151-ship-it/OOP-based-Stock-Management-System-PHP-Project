...
$accountService = new CustomerAccountService($conn);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($phone) || empty($password)) {
        ...
        $email_exists = false;
        if (!empty($email)) {
            $email_exists = $accountService->emailExists($email);
        }

        if ($accountService->phoneExists($phone_clean)) {
            ...
            $registerResult = $accountService->registerNewCustomer($name, $email, $phone_clean, $password);
            if (!empty($registerResult['ok'])) {
                header('Location: dashboard.php');
                exit;
            }

            $message = $registerResult['error'] ?? 'Error creating account. Please try again.';
            $message_type = 'error';
        }
    }
}
