...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $result = $registrationService->initiatePaidRegistration([
            'name' => $name,
            'phone' => $phone,
            'email' => $email,
            'nid' => $nid,
            'password' => $password,
            'type' => $customer_type,
            'fee' => (float)$registration_fee,
            'cancel_path' => '/customer/register_vip.php?cancelled=1',
        ]);
        ...
        $errors[] = $result['error'] ?? 'Payment gateway error: Please try again';
    }
}
