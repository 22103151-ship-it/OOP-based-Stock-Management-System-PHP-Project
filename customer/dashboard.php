...
$customer_id = (int)Auth::customerId();
$customer = $portalService->resolveCustomerByUserId((int)($_SESSION['user_id'] ?? 0));

if (!$customer && $customer_id > 0) {
    $customer = ['id' => $customer_id, 'name' => $_SESSION['name'] ?? 'Customer', 'customer_type' => $_SESSION['customer_type'] ?? 'pro'];
}
...
$product_count = $portalService->getAvailableProductCount();

// Get customer order statistics
$order_stats = $portalService->getOrderStats($customer_id);

// Get unread notifications count
$unread_notifications = $portalService->getUnreadNotificationsCount($customer_id);

// Get recent notifications
$recent_notifications = $portalService->getRecentNotifications($customer_id, 5);
?>
