...
$membershipService = new CustomerMembershipService($conn);

// Get customer info
$customer = $membershipService->getCustomer($customer_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_membership'])) {
    ...
    if ($amount < 100) {
        ...
        $payment = $membershipService->createPendingMembershipPayment($customer_id, $amount);
        if (!($payment['ok'] ?? false)) {
            $message = $payment['error'] ?? 'Unable to start membership payment.';
            $message_type = 'error';
        } else {
